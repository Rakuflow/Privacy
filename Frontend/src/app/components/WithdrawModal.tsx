              ...
              <div className="mt-2 p-2 bg-white/5 border border-white/10 rounded">
                ...
                <div className="mt-1.5 p-1.5 bg-blue-500/10 border border-blue-500/20 rounded flex gap-1.5">
                  ...
                  <p className="text-xs text-blue-300">
                    Multi-note: Auto combines notes if needed (multiple txs,
                    higher gas)
                  </p>
                </div>
              </div>
          ...
          {proofGenerated && useMultiNote && selectedNotes.length > 0 && (
            <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-lg">
              ...
              <p className="text-xs text-amber-200/80">
                This will execute {selectedNotes.length} separate withdrawals
                (higher gas cost)
              </p>
            </div>
          )}
          ...
          {!loading && !proofGenerated && steps[2].status === "pending" && (
            <div className="p-3 bg-purple-500/10 border border-purple-500/20 rounded-lg">
              <p className="text-xs text-purple-300">
                ⚠️ Withdrawing will unshield funds. On-chain observers will see
                recipient & amount, but not which note was spent.
              </p>
            </div>
          )}
                ...
                >
                  {loading && proofGenerated ? (
                    <>
                      ...
                      <span>
                        {useMultiNote
                          ? `Executing ${selectedNotes.length} Withdrawals...`
                          : "Submitting..."}
                      </span>
                    </>
                  ) : (
                    `Execute Withdrawal${
                      useMultiNote ? ` (${selectedNotes.length} notes)` : ""
                    }`
                  )}
                </GlowButton>
              </>
