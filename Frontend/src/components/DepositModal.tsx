...
export function DepositModal({ open, onOpenChange }: DepositModalProps) {
  ...
  const { amount, setAmount, loading, steps, formattedBalance, balance, keypair, isReady, handleDeposit, resetForm, isCompleted } = useDeposit(open);
          ...
          <div className="space-y-4 mt-4">
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <button
                  onClick={() => setAmount((Number(balance) / 10 ** TOKENS.STRK.decimals).toString())}
              ...
              <Input type="number" placeholder="0.00" value={amount} onChange={(e) => setAmount(e.target.value)} className="bg-white/5 border-white/10" disabled={isCompleted} />
            </div>
            ...
            <GlowButton
              className="w-full"
              disabled={isCompleted ? false : !amount || loading || !keypair || !isReady || steps[1].status === 'completed' || steps[2].status === 'active'}
