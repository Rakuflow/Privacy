 ...
 */
export async function syncNotesWithChain(zkAddress: string, userSpendingKey: string, onProgress?: (progress: { current: number; total: number; message: string }) => void): Promise<SyncResult> {
  try {
    ...
    for (let i = 0; i < localNotes.length; i++) {
      ...
      if (leafIndex !== undefined) {
        ...
        const noteSpendingKey = resolveNoteSpendingKey(note, userSpendingKey, zkAddress);
 ...
 */
export async function scanForReceivedNotes(zkAddress: string, spendingKey: string, onProgress?: (progress: { current: number; total: number; message: string }) => void): Promise<number> {
  try {
