...
import type { DepositEventData, TransferEventData, WithdrawEventData } from '../types/EventScanner.type';

/**
 * Scan DepositEvent from contract
 */
export async function scanDepositEvents(fromBlock: number = 0, toBlock: number | 'latest' = 'latest'): Promise<DepositEventData[]> {
  try {
 ...
 */
export async function scanTransferEvents(fromBlock: number = 0, toBlock: number | 'latest' = 'latest'): Promise<TransferEventData[]> {
  try {
    const provider = new Provider({ nodeUrl: NETWORK.RPC_URL });
    ...
    const events = await provider.getEvents({
      ...
      from_block: { block_number: fromBlock },
      to_block: toBlock === 'latest' ? 'latest' : { block_number: toBlock },
      keys: [[hash.getSelectorFromName('TransferEvent')]],
      chunk_size: 1000,
    });
 ...
 */
export async function scanWithdrawEvents(fromBlock: number = 0, toBlock: number | 'latest' = 'latest'): Promise<WithdrawEventData[]> {
  try {
    const provider = new Provider({ nodeUrl: NETWORK.RPC_URL });
    ...
    const events = await provider.getEvents({
      ...
      from_block: { block_number: fromBlock },
      to_block: toBlock === 'latest' ? 'latest' : { block_number: toBlock },
      keys: [[hash.getSelectorFromName('WithdrawEvent')]],
      chunk_size: 1000,
    });
 ...
 */
export async function getAllCommitmentsOnChain(fromBlock: number = 0): Promise<Map<string, string>> {
  const [deposits, transfers] = await Promise.all([scanDepositEvents(fromBlock), scanTransferEvents(fromBlock)]);
 ...
 */
export async function getSpentNullifiers(fromBlock: number = 0): Promise<Set<string>> {
  const [withdraws, transfers] = await Promise.all([scanWithdrawEvents(fromBlock), scanTransferEvents(fromBlock)]);
